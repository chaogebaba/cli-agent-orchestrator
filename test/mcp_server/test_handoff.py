...
class TestHandoffOutcomes:
    ...
    @patch("cli_agent_orchestrator.mcp_server.server._resolve_handoff_provider")
    def test_endpoint_409_waiting_user_input_reports_blocked_worker(self, mock_provider):
        mock_provider.return_value = _ctx("codex")
        blocked = MagicMock()
        blocked.status_code = 409
        blocked.json.return_value = {
            "detail": {
                "message": "terminal a1b2c3d4 is waiting for user input",
        ...
        assert result.success is False
        assert result.terminal_id == "a1b2c3d4"
        assert "blocked: worker is waiting for user input" in result.message
