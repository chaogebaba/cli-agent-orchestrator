...
- Instead we drive the REAL assembled ASGI app via Starlette's ``TestClient`` used as a
  ...
  ``cancel_signal`` EXACTLY as the real substrate does; the engine drive, journal, and
...
def _long_yaml_leaf() -> Callable:
    """A stubbed ``run_agent_step`` that stays in-flight until cancel_signal fires.

    Honors ``cancel_signal`` (canonical name; workflow_service passes it) EXACTLY
    as the real substrate does: it raises ``StepCancelledError`` the instant the
    engine sets the event, so the real drive converges the run to CANCELLED
    through its real code path (not a shortcut).

    Note: the real ``run_agent_step`` also accepts upstream alias ``cancel_event``,
    but a monkeypatched substitute does NOT get that alias — read the canonical
    kwarg the engine actually passes.
    """
