...
class TestRunStepEndpoint:
    ...
    def test_reused_ready_base_refresh_authorization_allows_step(self, client):
        ...
        with (
            ...
            patch(_RUN_STEP, new=AsyncMock(return_value=result)) as run,
        ):
            resp = client.post(
                TERMINALS_RUN_STEP_ROUTE,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "claude-opus-5[1m]",  # explicit overrides use the conservative charset
    ...
    def test_waiting_user_input_maps_to_409_with_structured_terminal_id(self, client):
        with patch(
            _RUN_STEP,
            new=AsyncMock(
                side_effect=StepExecutionError(
                    "terminal abc12345 is waiting for user input",
        ...
        assert resp.status_code == 409
        assert resp.json()["detail"] == {
            "message": "terminal abc12345 is waiting for user input",
            "kind": "waiting_user_input",
            "terminal_id": "abc12345",
        }
