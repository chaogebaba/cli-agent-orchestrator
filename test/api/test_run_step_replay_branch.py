...
class TestTheRouteDecidesNothing:
    ...
    def test_the_route_logs_only_inside_best_effort_bookkeeping_guards(self):
        ...
        assert calls == ["logger.warning", "logger.warning", "logger.exception"]
        # The two warning calls are inside best-effort bookkeeping guards.
        # The exception call is the OutputExtractionError 500 handler (issue #570).
        # None are on the REPLAY path itself (SR-7's noise-and-leak surface).
        assert "logger.info" not in code
        assert "logger.debug" not in code
        for line in code.splitlines():
            if "logger." in line:
                assert "exc_info=True" in line or "bookkeeping" in line or "logger.exception" in line
