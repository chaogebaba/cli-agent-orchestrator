...
class TestScreenDetection:
    ...
    @patch("cli_agent_orchestrator.services.status_monitor.CAO_PYTE_STATUS", True)
    @patch("cli_agent_orchestrator.services.status_monitor.provider_manager")
    async def test_ready_bursting_codex_one_line_spinner_repaint_recovers_processing(self, mock_pm):
...
class TestStatusGenerations:
    def test_no_change_ready_acceptance_is_logged(self, caplog):
        ...
        assert (
            "accepted completed generation: input_gen=0 processing_gen=0 status_gen=0"
            in caplog.text
        )
    ...
    def test_generation_acceptance_paths_are_logged(self, caplog):
        ...
        assert (
            "accepted processing generation: input_gen=1 processing_gen=1 status_gen=0"
            in caplog.text
        )
        assert (
            "accepted completed generation: input_gen=1 processing_gen=1 status_gen=1"
            in caplog.text
        )
    ...
    def test_notify_invalidates_detection_computed_at_live_sequence(self):
        ...
        m.sm._apply_detection("t1", TerminalStatus.PROCESSING, expected_seq=detection_seq)
