...
def test_wpm1_merge_before_cas_rolls_back_on_member_mismatch(wpm1_db):
    ...
    assert (
        settle_wpm1_terminal_batch([message.id, 999], MessageStatus.DELIVERED, "receiver")
        == "stale"
    )
    with wpm1_db() as db:
...
def test_wpm1_stalled_notice_is_atomic_and_external_retry_deduplicates(wpm1_db):
    ...
    assert (
        record_wpm1_stalled_notice(attempt, [message.id], "receiver", stamp) == "already_recorded"
    )
    with wpm1_db() as db:
        notices = (
            db.query(InboxModel)
        ...
        assert (
            json.loads(db.get(InboxDeliveryAttemptModel, attempt).evidence)["stalled_notified_at"]
            == stamp
        )
...
def test_wpm1_notice_header_lf_prevents_one_ten_collision(wpm1_db):
    ...
    assert (
        record_wpm1_stalled_notice(attempt, [one.id], "receiver", "2026-07-13T01:02:03Z")
        == "recorded"
    )
    with wpm1_db() as db:
        bodies = [
            ...
            .filter(InboxModel.sender_id == "message-trace:receiver")
            .all()
        ]
...
def test_wpm1_concurrent_independent_stalled_writers_insert_once(wpm1_db):
    ...
    def worker():
        barrier.wait()
        outcomes.append(
            record_wpm1_stalled_notice(attempt, [message.id], "receiver", "2026-07-13T01:02:03Z")
        )
...
def test_wpm1_late_confirm_writes_clock_and_corrective_to_durable_recipient(wpm1_db):
    ...
    record_wpm1_stalled_notice(attempt, [message.id], "receiver", "2026-07-13T01:02:03Z")
    assert (
        settle_wpm1_terminal_batch([message.id], MessageStatus.DELIVERED, "receiver") == "settled"
    )
    with wpm1_db() as db:
...
def test_wpm1_log_only_stall_late_confirm_logs_without_orphan(wpm1_db, caplog):
    ...
    with wpm1_db.begin() as db:
        db.query(database.TerminalModel).filter(
            database.TerminalModel.id.in_(("sender", "caller"))
...
def test_wpm1_stalled_pair_crash_rolls_back_after_every_write(wpm1_db, hook, statement):
    message, attempt = _ambiguous()
    engine = wpm1_db.kw["bind"]
    fired = False
...
def test_wpm1_corrective_pair_crash_rolls_back_after_every_write(wpm1_db, hook, statement):
    message, attempt = _ambiguous()
    record_wpm1_stalled_notice(attempt, [message.id], "receiver", "2026-07-13T01:02:03Z")
    engine = wpm1_db.kw["bind"]
    fired = False
...
def test_wpm1_cleanup_day15_retains_unsettled_batch_and_notice(
    ...
    message, attempt = _ambiguous()
    record_wpm1_stalled_notice(attempt, [message.id], "receiver", "2026-06-01T01:02:03Z")
    with wpm1_db.begin() as db:
...
def test_wpm1_cleanup_day15_external_retry_keeps_dedup_key(wpm1_db, monkeypatch, tmp_path):
    from cli_agent_orchestrator.services import cleanup_service
...
def test_wpm1_cleanup_reaps_terminal_batch_attempt_and_notice_together(
    ...
    message, attempt = _ambiguous()
    assert (
        record_wpm1_stalled_notice(attempt, [message.id], "receiver", "2026-01-01T01:02:03Z")
        == "recorded"
    )
    assert (
        settle_wpm1_terminal_batch([message.id], MessageStatus.DELIVERED, "receiver") == "settled"
...
def test_wpm1_all_terminal_arms_write_canonical_clock(wpm1_db, status, reason):
    message, attempt = _ambiguous()
    assert settle_wpm1_terminal_batch([message.id], status, "receiver", reason=reason) == "settled"
    with wpm1_db() as db:
        evidence = json.loads(db.get(InboxDeliveryAttemptModel, attempt).evidence)
        assert evidence["terminal_settled_at"].endswith("Z")
...
def test_wpm1_watchdog_callback_status_query_exact_matrix(wpm1_db):
    ...
    cases = [
        ...
        ("sender", "receiver", timedelta(microseconds=1), MessageStatus.DELIVERY_FAILED, None),
        ("other", "receiver", timedelta(microseconds=1), MessageStatus.PENDING, None),
        ("watchdog:sender", "receiver", timedelta(microseconds=1), MessageStatus.PENDING, None),
        ("sender", "caller", timedelta(microseconds=1), MessageStatus.PENDING, None),
        ("sender", "receiver", -timedelta(microseconds=1), MessageStatus.PENDING, None),
...
def test_wpm1_watchdog_pending_callback_failure_transition_fires_once(wpm1_db, failure_status):
    watchdog = StalledCallbackWatchdog(grace_seconds=3)
    watchdog.record_inbound_task("sender", "receiver", "developer")
...
@pytest.mark.parametrize("failure_status", [MessageStatus.FAILED, MessageStatus.DELIVERY_FAILED])
def test_wpm1_watchdog_terminal_failure_only_callback_fires_once(wpm1_db, failure_status):
    watchdog = StalledCallbackWatchdog(grace_seconds=3)
    watchdog.record_inbound_task("sender", "receiver", "developer")
    message = create_inbox_message("sender", "receiver", "callback")
    with wpm1_db.begin() as db:
...
def lawful_boundary_observation(
    ...
    *,
    ...
    observation = BoundaryObservation(epoch, status, 3, 1, seq, non_ready_seq, ready_seq)
    target = evidence
    if attempt is not None:
        target = json.loads(attempt["evidence"])
    if target is not None:
        target["injection_completed_seq"] = {"observation_epoch": epoch, "seq": anchor_seq}
        if exhausted:
...
def _binding():
    return TranscriptResolution(
        ...
        resolution_kind="binding",
        live_reference=TranscriptLiveReference(__import__("pathlib").Path("/trace"), 1, 20),
    )
...
def test_incident2_replay_growth_injects_once_then_d2_confirms(wpm1_db):
    ...
    with (
        ...
        patch(
            "cli_agent_orchestrator.services.message_trace_service.continuity_aware_lookup",
            side_effect=[("absent", {"size": 999}), ("absent", {"size": 1999}), ("hit", {})],
        ),
        patch("cli_agent_orchestrator.services.inbox_service.status_monitor") as monitor,
        patch(
...
def test_busy_wake_after_third_injection_cannot_cap():
    ...
    provider.read_composer_draft_state.return_value = "empty"
    attempts = [
        _gate_attempt(0, exhausted=True),
        _gate_attempt(1, exhausted=True),
        _gate_attempt(2),
    ]
    observation = lawful_boundary_observation(TerminalStatus.PROCESSING)
    for index, attempt in enumerate(attempts):
        lawful_boundary_observation(TerminalStatus.COMPLETED, attempt=attempt, exhausted=index < 2)
    with (
...
def test_stalled_notice_fires_at_30min_idle_or_4h_absolute(age_minutes, activity_age):
    ...
    evidence.update(
        {
            "last_activity_at": (datetime.now(timezone.utc) - timedelta(minutes=activity_age))
            .isoformat()
            .replace("+00:00", "Z"),
    ...
    with (
        ...
        patch(
            "cli_agent_orchestrator.services.inbox_service.record_wpm1_stalled_notice",
...
def test_exact_batch_attempt_lookup_rejects_overlapping_member_set():
    ...
    with (
        patch(
            "cli_agent_orchestrator.services.inbox_service.list_message_attempts",
            return_value=attempts,
        ),
        patch(
            "cli_agent_orchestrator.services.inbox_service.list_attempt_member_ids",
            side_effect=[[1], [1, 2]],
        ),
    ):
...
def test_successor_writer_pins_exact_exhausted_source_despite_newer_overlap(wpm1_db):
    ...
    first = next(
        message for message in get_pending_messages("receiver", limit=10) if message.id == first.id
    )
...
def test_successor_restart_after_exhaustion_merge_injects_once(wpm1_db):
    ...
    with (
        ...
        patch(
            ...
            return_value=(
                "absent",
                {"path": "/trace", "inode": 1, "size": 10, "resolution_kind": "binding"},
            ),
        ),
        patch("cli_agent_orchestrator.services.inbox_service.status_monitor") as monitor,
        patch(
    ...
    with wpm1_db() as db:
        attempts = (
            db.query(InboxDeliveryAttemptModel).order_by(InboxDeliveryAttemptModel.started_at).all()
        )
...
def test_interrupted_successor_blocks_duplicate_respawn():
    ...
    interrupted.update(
        outcome="interrupted",
        reason="pane_unresolvable",
        prior_attempt_uuid=exhausted["attempt_uuid"],
    )
    observation = lawful_boundary_observation(TerminalStatus.IDLE)
    lawful_boundary_observation(TerminalStatus.IDLE, attempt=exhausted, exhausted=True)
    provider = MagicMock()
    provider.read_composer_draft_state.return_value = "empty"
    with (
...
def test_ambiguous_attempt_queued_command_confirmation_stops_reinjection(tmp_path):
    ...
    def lookup(_metadata, payload_hash, started_at, _expected_ref):
        return transcript_lookup(transcript, payload_hash, started_at, scan_from_start=True)
...
@pytest.mark.parametrize(
    ...
    [
        "{}",
        "{bad-json",
        json.dumps({"resolution_kind": "binding", "stale_note": "binding_stale:missing"}),
    ],
)
