...
def test_resume_prepublication_failures_release_all_locally_owned_authority(
    ...
    if failure == "owner_query":
        ...
        monkeypatch.setattr(
            terminal_service,
            "load_agent_profile",
            lambda _p: AgentProfile(name="dev", description="", sessionBrief="required"),
        )
...
def test_invalid_supplied_uuid_token_releases_locally_owned_lifecycle(monkeypatch):
    ...
    context = ForkContext(
        mode="resume",
        session_uuid="wanted-prepub-uuid",
        base_name="b",
        provider="grok_cli",
        initial_preamble="",
    )
    with pytest.raises(RuntimeError, match="invalid_provider_session_lease_token"):
        asyncio.run(
            terminal_service.create_terminal(
                "grok_cli",
                "dev",
                session_name="cao-invalid-prepub",
                fork_context=context,
                uuid_lease_token=wrong,
            )
        )
...
def test_seed_required_is_before_every_side_effect(monkeypatch):
    ...
    monkeypatch.setattr(
        terminal_service,
        "get_provider_class",
        lambda _p: SimpleNamespace(supports_seed_resume_identity=True),
    )
...
def test_resume_publication_and_identity_confirmation(monkeypatch):
    ...
    # F786 (#643) D8: a NAMED profile ("dev") must load or create_terminal fails
    # closed (E-PROFILE-MISSING). This test is about resume publication/identity,
    # so give it a loadable profile rather than the pre-F786 missing→None path.
    monkeypatch.setattr(
        terminal_service,
        "load_agent_profile",
        MagicMock(return_value=AgentProfile(name="dev", description="Developer")),
    )
    ...
    monkeypatch.setattr(terminal_service, "list_terminals_by_provider_session_id", lambda _u: [])
    monkeypatch.setattr(
        terminal_service.provider_manager, "create_provider", lambda *_a, **_k: provider
    )
    monkeypatch.setattr(
    ...
    result = asyncio.run(
        terminal_service.create_terminal("grok_cli", "dev", new_session=True, fork_context=context)
    )
...
def test_published_failure_waits_for_public_teardown_holder_and_settles(
    ...
    thread = threading.Thread(
        target=lambda: result.update(
            terminal_service._settle_published_creation_failure(terminal_id, uuid, uuid_token, None)
        )
    )
    thread.start()
    time.sleep(0.03)
...
def test_live_other_owner_conflicts_even_under_unrelated_rebind_lease(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "get_terminal_metadata",
        lambda _tid: {
            "provider_session_id": "fallback-live",
...
def test_fallback_stops_before_settlement_with_all_four_effects_absent(real_db, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="crash_before_settle"):
        asyncio.run(
            provider_rebind_service._fallback(
...
def test_fallback_call_graph_never_runs_seed_resume_bootstrap(monkeypatch):
    ...
    assert lifecycle_lease is not None
    result = asyncio.run(
        provider_rebind_service._fallback(metadata, "uuid", object(), lifecycle_lease)
    )
    release_session_lifecycle_lease(lifecycle_lease)
...
@pytest.mark.parametrize(
    ...
    [("live", False, False), ("error", False, False), ("gone", False, True), ("gone", True, True)],
)
def test_owner_preflight_matrix(monkeypatch, liveness, quarantined, proceeds):
    ...
    monkeypatch.setattr(terminal_service, "get_backend", lambda: backend)
    # F786 (#643) D8: the NAMED profile ("dev") loads BEFORE generate_terminal_id,
    # so the proceed cases must supply a loadable profile or create_terminal fails
    # closed (E-PROFILE-MISSING) before the preflight_passed sentinel can fire.
    monkeypatch.setattr(
        terminal_service,
        "load_agent_profile",
        MagicMock(return_value=AgentProfile(name="dev", description="Developer")),
    ...
    context = ForkContext(
        mode="resume", session_uuid=uuid, base_name="b", provider="grok_cli", initial_preamble=""
    )
...
def test_sync_failure_racing_public_teardown_holder_retries_and_releases_uuid(monkeypatch):
    ...
    monkeypatch.setattr(terminal_service, "get_backend", lambda: backend)
    # F786 (#643) D8: a NAMED profile must load or create_terminal fails closed
    # before init — this test is about a sync-failure/teardown race, so give it a
    # loadable profile rather than the pre-F786 missing→None path.
    monkeypatch.setattr(
        terminal_service,
        "load_agent_profile",
        MagicMock(return_value=AgentProfile(name="dev", description="Developer")),
...
@pytest.mark.asyncio
async def test_deferred_failure_racing_session_teardown_holder_notifies_after_settlement(
    monkeypatch,
):
    ...
    terminal_service._schedule_deferred_init(
        ...
        None,
        None,
        uuid_lease_token=uuid_token,
        owns_uuid_lease=True,
        settlement_form="resume",
    )
