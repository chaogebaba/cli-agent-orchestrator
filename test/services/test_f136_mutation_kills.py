...
def _seed(sessions, *, mailbox_id="mb_sup", terminal_id="t1", generation=1,
           cursor=None, path="/tmp/inbox.json", path_version=0):
...
def _inbox_row(sessions, row_id, *, mailbox_id="mb_sup", terminal_id="t1",
    ...
    def test_attempt_teammate_push_on_insert_retired_with_metadata(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda tid: {
                ...
                "provider": "claude_code", "agent_profile": "sup",
        ...
        msg = InboxMessage(
            id=42, sender_id="worker-1", receiver_id="sup-001",
            message="task done", orchestration_type=OrchestrationType.SEND_MESSAGE,
            status=MessageStatus.PENDING, created_at=_NOW,
        )
...
class TestM4:
    def test_forward_stream_excludes_at_and_below_cursor(self, f136_sessions):
        ...
        batch = get_supervisor_callback_batch(
            mailbox_id="mb_sup", terminal_id="t1", generation=1, limit=50,
        )
    ...
    def test_cancel_barrier_enqueues_replay_for_released_below_cursor(self, f136_sessions):
        ...
        with sessions.begin() as db:
            barrier = CallbackBarrierModel(
                owner_mailbox_id="mb_sup", owner_terminal_id=None,
                owner_generation=1, label="test-gate",
                state="OPEN", timeout_at=_NOW + timedelta(hours=1),
                created_at=_NOW,
            )
            db.add(barrier)
            db.flush()
            ...
            db.add(InboxModel(
                id=5, sender_id="worker-1", receiver_id="t1",
                logical_receiver_id="mb_sup", message="held-msg",
                orchestration_type="send_message", status="held",
                enqueue_generation=1, barrier_id=barrier_id,
                created_at=_NOW,
            ))
            db.add(CallbackBarrierMemberModel(
                barrier_id=barrier_id, member_key="worker-1",
                position=1, terminal_id="t1", lifecycle_generation=1,
                state="AWAITING",
            ))
        ...
        with sessions() as db:
            replay = db.execute(text(
                "SELECT inbox_row_id FROM callback_replay_queue WHERE mailbox_id='mb_sup'"
            )).fetchall()
...
class TestM6:
    def test_settle_terminal_rebound_enqueues_replay_below_cursor(self, f136_sessions):
        ...
        with sessions.begin() as db:
            db.add(InboxModel(
                id=3, sender_id="worker-1", receiver_id="t1",
                logical_receiver_id="mb_sup", message="parked-msg",
                orchestration_type="send_message", status="parked",
                enqueue_generation=1,
                owner_receiver_id="t1", owner_generation=1,
                created_at=_NOW,
            ))
    ...
    def test_path_change_enqueues_replay_for_below_cursor_rows(self, f136_sessions):
        ...
        with patch("cli_agent_orchestrator.services.inbox_service.request_delivery"):
            result = set_supervisor_callback_inbox_path(
                mailbox_id="mb_sup", terminal_id="t1", generation=1,
                path="/new/path",
            )

        assert result.kind == "updated"
        with sessions() as db:
            replay_ids = [r[0] for r in db.execute(text(
                "SELECT inbox_row_id FROM callback_replay_queue WHERE mailbox_id='mb_sup'"
            )).fetchall()]
...
class TestM9:
    def test_bootstrap_cursor_law_excludes_stale_generation_rows(self, f136_sessions):
        ...
        batch = get_supervisor_callback_batch(
            mailbox_id="mb_sup", terminal_id="t1", generation=2, limit=50,
        )
        assert batch.kind == "ok"
...
class TestM17:
    def test_commit_progress_deletes_replay_atomically(self, f136_sessions):
        ...
        result = commit_supervisor_callback_progress(
            mailbox_id="mb_sup", terminal_id="t1", generation=1,
            expected_cursor=5, new_cursor=5,  # equal = replay-only
    ...
    def test_commit_progress_fails_on_stale_authority_no_deletion(self, f136_sessions):
        ...
        result = commit_supervisor_callback_progress(
            mailbox_id="mb_sup", terminal_id="t1", generation=99,
...
class TestM20:
    def test_request_delivery_posts_outside_guard(self):
        ...
        try:
            ...
            assert guard_held_during_post[0] is False, "call_soon_threadsafe called while guard held!"
        finally:
        ...
        try:
            ...
            with _delivery_seq_guard:
                _wake_states[terminal] = _WakeState(
                    dirty_epoch=5, immediate_admitted=True, holder_epoch=5,
                )
            request_delivery(terminal)
...
class TestM25:
    def test_post_delivery_reruns_when_dirty_gt_holder(self):
        ...
        from cli_agent_orchestrator.services.inbox_service import (
            ...
            CallbackRunOutcome, inbox_service,
        )
        ...
        try:
            ...
            with _delivery_seq_guard:
                _wake_states[terminal] = _WakeState(
                    dirty_epoch=10, immediate_admitted=True, holder_epoch=5,
                )
