...
def test_the_measurement_does_not_move_with_the_checkout_path(tmp_path: Path) -> None:
    """compose.py writes the ABSOLUTE manifest path into its GENERATED header.

    Measured 2026-09-16: the same doctrine composed to 44,610 bytes under
    ``/home/chao/VScode_projects/cli-subagents`` and 44,617 under a worktree, purely
    from the path in that header. Two checkouts of one tree must measure the same.
    """
    short = _workspace(tmp_path)
    deep = tmp_path / "a" / "much" / "deeper" / "checkout"
    deep.mkdir(parents=True)
    import shutil as _shutil

    _shutil.copytree(short, deep / "ws")
    sizes = []
    for root in (short, deep / "ws"):
        result = _lint(root, tmp_path)
        sizes.append([row.size for row in result.measurements if row.name == "playbook:demo"][0])
    assert sizes[0] == sizes[1], f"the budget moved with the path: {sizes}"


def test_normalize_composed_only_rewrites_the_manifest_path(tmp_path: Path) -> None:
    manifest = tmp_path / "doctrine/manifests/orchestrator.toml"
    manifest.parent.mkdir(parents=True)
    manifest.touch()
    header = f"<!-- GENERATED by compose.py from {manifest}; edit the sections -->\nbody\n"
    out = normalize_composed(header.encode(), tmp_path, manifest)
    assert out.decode() == (
        "<!-- GENERATED by compose.py from doctrine/manifests/orchestrator.toml;"
        " edit the sections -->\nbody\n"
    )
    assert normalize_composed(b"no path here\n", tmp_path, manifest) == b"no path here\n"
