...
def test_verify_manifest_regen_preserves_identities_across_line_shift(
    ...
    assert result.exit_code == 0
    assert result.output == "Trace manifest: hits=36 files_touched=1 changed=yes\n"
    ...
    assert len(committed_rows) == len(regenerated_rows) == 36
...
def test_verify_manifest_regen_unchanged_tree_does_not_write(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 0
    assert result.output == "Trace manifest: hits=36 files_touched=0 changed=no\n"
    assert writes == []
