...
def claim_mailbox(session_name: str, role: str = "supervisor") -> MailboxClaim:
    with SessionLocal() as db:
        row: Any = db.query(MailboxModel).filter_by(
            session_name=session_name, role=role
        ).one_or_none()
        return MailboxClaim(
...
def _acquire(lock: threading.Lock) -> None:
    if not lock.acquire(timeout=MAILBOX_AUTHORITY_TIMEOUT_SECONDS):
        raise MailboxDomainError(
            "mailbox_authority_timeout", "mailbox authority lock timed out"
        )


def _address_ids(db: Any, mailbox_id: str) -> list[str]:
    return [row[0] for row in db.query(MailboxIncarnationModel.terminal_id).filter_by(
...
def publish_supervisor_incarnation(claim: MailboxClaim, terminal_id: str) -> dict[str, Any]:
    ...
    try:
        with SessionLocal() as db:
            ...
            existing = db.query(MailboxIncarnationModel).filter_by(
            ...
            if claim.mailbox_id is None:
                mailbox = MailboxModel(
                    id=f"mb_{uuid.uuid4().hex[:8]}", session_name=claim.session_name,
                    role=claim.role, current_terminal_id=terminal_id, generation=1,
                    consumed_through_id=0, created_at=now, updated_at=now,
                )
                db.add(mailbox)
                try:
                    db.flush()
                ...
                changed = db.query(MailboxModel).filter(
            ...
            incarnation = MailboxIncarnationModel(
                mailbox_id=mailbox.id, generation=generation,
                terminal_id=terminal_id, published_at=now,
            )
            db.add(incarnation)
            db.flush()
            address_ids = _address_ids(db, mailbox.id)
            pending: list[Any] = db.query(InboxModel).filter(
                InboxModel.status == MessageStatus.PENDING.value,
                or_(
                    InboxModel.logical_receiver_id == mailbox.id,
                    and_(InboxModel.logical_receiver_id.is_(None),
                         InboxModel.receiver_id.in_(address_ids)),
                ),
            ).all()
            wake_ids = sorted({row.receiver_id for row in pending} | {terminal_id})
            for row in pending:
                row.receiver_id = terminal_id
                if row.logical_receiver_id is None:
                    row.logical_receiver_id = mailbox.id

            delivered = (db.query(InboxModel).filter(
                ...
                or_(InboxModel.logical_receiver_id == mailbox.id,
                    InboxModel.receiver_id.in_(address_ids)),
            ).order_by(InboxModel.id.asc()).all())
            digest_id = None
            if delivered:
                first, last = delivered[0], delivered[-1]
                digest = InboxModel(
                    sender_id="mailbox-digest", receiver_id=terminal_id,
                    logical_receiver_id=mailbox.id,
                    message=(f"mailbox digest: {len(delivered)} delivered message(s), "
                             f"ids {first.id}-{last.id}, senders "
                             f"{first.sender_id}..{last.sender_id}"),
            ...
            return {
                "mailbox_id": mailbox.id, "generation": generation,
                "digest_message_id": digest_id, "adopted_receiver_ids": wake_ids,
            }
    except IntegrityError as exc:
...
def create_logical_inbox_message(
    *, sender_id: str, mailbox_id: str, message: str, refresh_ingest: bool = False,
    ...
    try:
        with SessionLocal() as db:
            receiver_cache, logical_receiver_id = resolve_inbox_receiver(db, mailbox_id)
            ...
            row: Any = InboxModel(
                sender_id=sender_id, receiver_id=receiver_cache,
                logical_receiver_id=logical_receiver_id, message=message,
            ...
            return InboxMessage(
                ...
                logical_receiver_id=row.logical_receiver_id, message=row.message,
                orchestration_type=OrchestrationType(row.orchestration_type),
                status=MessageStatus(row.status), failure_reason=row.failure_reason,
...
def acquire_logical_sender_authority(
    mailbox_id: str, receiver_terminal_id: str, expected_generation: int,
) -> threading.Lock | None:
...
def _attempt_outcome(db: Any, message_id: int) -> str:
    attempt = (db.query(InboxDeliveryAttemptModel)
               ...
               .order_by(InboxDeliveryAttemptModel.started_at.desc(),
                         InboxDeliveryAttemptModel.attempt_uuid.desc()).first())
    if attempt is None:
        return "none"
    if attempt.outcome == "confirmed":
        try:
            ...
            evidence = {}
        return ("confirmed_unverified" if evidence.get("kind") == "send_returned_unverified"
                else "confirmed_hit")
    return attempt.outcome if attempt.outcome in {
        "ambiguous", "failed", "deferred", "interrupted", "unresolved"
    } else "none"


def list_messages(
    receiver: str, *, since: datetime | None = None, after_id: int | None = None,
    limit: int = 25, status: MessageStatus | None = None,
) -> dict[str, Any]:
    with SessionLocal() as db:
        ...
        if receiver.startswith("mb_"):
            ...
            query = query.filter(or_(InboxModel.logical_receiver_id == receiver,
        ...
        items = [{
            "id": row.id, "sender_id": row.sender_id, "receiver_id": row.receiver_id,
            "logical_receiver_id": row.logical_receiver_id, "message": row.message,
            "orchestration_type": row.orchestration_type, "status": row.status,
            "failure_reason": row.failure_reason,
            "last_attempt_outcome": _attempt_outcome(db, row.id),
            "created_at": row.created_at.isoformat(),
        } for row in rows]
        return {"items": items, "next_after_id": rows[-1].id if has_more else None,
                "has_more": has_more}


def ack_messages(terminal_id: str, up_to_id: int) -> dict[str, Any]:
    with SessionLocal() as db:
        mailbox: Any = db.query(MailboxModel).filter_by(
        ...
        key, observed_generation, mailbox_id = (
            (mailbox.session_name, mailbox.role), mailbox.generation, mailbox.id
        )
    lock = get_mailbox_authority_lock(*key)
    _acquire(lock)
    try:
        with SessionLocal() as db:
            ...
            high_water = db.query(func.max(InboxModel.id)).filter(or_(
                InboxModel.logical_receiver_id == mailbox.id,
                InboxModel.receiver_id.in_(addresses),
            )).scalar() or 0
            ...
            if up_to_id < mailbox.consumed_through_id:
                db.commit()
                return {"mailbox_id": mailbox.id,
                        "consumed_through_id": mailbox.consumed_through_id, "changed": False}
            changed = db.query(MailboxModel).filter(
                MailboxModel.id == mailbox.id,
                MailboxModel.generation == observed_generation,
                MailboxModel.current_terminal_id == terminal_id,
                MailboxModel.consumed_through_id <= up_to_id,
            ).update({MailboxModel.consumed_through_id: up_to_id,
                      MailboxModel.updated_at: datetime.now()}, synchronize_session=False)
            if changed != 1:
                raise MailboxDomainError("not_current_incarnation", "terminal is not current")
            prior = mailbox.consumed_through_id
            db.commit()
            return {"mailbox_id": mailbox.id, "consumed_through_id": up_to_id,
...
def list_mailboxes() -> dict[str, Any]:
    with SessionLocal() as db:
        rows: list[Any] = db.query(MailboxModel).order_by(
            MailboxModel.created_at, MailboxModel.id
        ).all()
        return {"items": [{
            "id": row.id, "session_name": row.session_name, "role": row.role,
            "current_terminal_id": row.current_terminal_id, "generation": row.generation,
            "consumed_through_id": row.consumed_through_id,
            "incarnation_count": db.query(MailboxIncarnationModel).filter_by(
                mailbox_id=row.id).count(),
        ...
        for attempt in range(3):
            try:
                with SessionLocal() as db:
                    ...
                    if mailbox is None:
                        raise MailboxDomainError("unknown_mailbox", "unknown mailbox")
                    if mailbox.current_terminal_id and db.query(TerminalModel).filter_by(
                        id=mailbox.current_terminal_id).first() is not None:
                        raise MailboxDomainError("mailbox_in_use", "mailbox is in use")
                    logical_ids = [row[0] for row in db.query(InboxModel.id).filter_by(
                        logical_receiver_id=mailbox_id).all()]
                    busy = db.query(InboxModel).filter(
                        InboxModel.id.in_(logical_ids),
                        InboxModel.status == MessageStatus.DELIVERING.value).first()
                    open_attempt = (db.query(InboxDeliveryAttemptModel)
                                    .join(InboxDeliveryAttemptMemberModel,
                                          InboxDeliveryAttemptMemberModel.attempt_uuid ==
                                          InboxDeliveryAttemptModel.attempt_uuid)
                                    .filter(InboxDeliveryAttemptMemberModel.message_id.in_(logical_ids),
                                            InboxDeliveryAttemptModel.settled_at.is_(None)).first())
                    if busy is not None or open_attempt is not None:
                        raise MailboxDomainError("mailbox_busy", "mailbox has an active delivery")
                    pending: list[Any] = db.query(InboxModel).filter_by(
                        logical_receiver_id=mailbox_id,
                        status=MessageStatus.PENDING.value).all()
                    notices = 0
                    settled: list[Any] = []
                    for row in pending:
                        ...
                        settled.append(row)
                        try:
                            receiver, logical = resolve_inbox_receiver(db, row.sender_id)
                        except ValueError:
                            continue
                        header = f"mailbox-delete receiver={mailbox_id} message={row.id}\n"
                        prior = db.query(InboxModel).filter(
                            InboxModel.sender_id == f"message-trace:{mailbox_id}",
                            InboxModel.receiver_id == receiver,
                            InboxModel.message.startswith(header)).first()
                        if prior is None:
                            db.add(InboxModel(
                                sender_id=f"message-trace:{mailbox_id}", receiver_id=receiver,
                                logical_receiver_id=logical, message=header +
                                "delivery failed because the logical mailbox was deleted",
                                orchestration_type=OrchestrationType.SEND_MESSAGE.value,
                                status=MessageStatus.PENDING.value,
                            ))
                            notices += 1
                    if db.query(InboxModel).filter(
                        InboxModel.logical_receiver_id == mailbox_id,
                        InboxModel.status == MessageStatus.DELIVERING.value).first() is not None:
                        raise MailboxDomainError("mailbox_busy", "mailbox has an active delivery")
                    db.query(MailboxIncarnationModel).filter_by(mailbox_id=mailbox_id).delete(
                        synchronize_session=False)
                    deleted = db.query(MailboxModel).filter(
