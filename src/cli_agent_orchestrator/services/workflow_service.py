...
NON_RETRYABLE_STEP_KINDS = frozenset({"input_blocked", "waiting_user_input", "cancelled"})
...
def _journal_run_state_snapshot(run_id: str, state: RunState, finished_at: Optional[str]) -> None:
    try:
...
def cancel_run(run_id: str) -> None:
    ...
    if run_id not in _cancel_recovery_tasks:
        _cancel_recovery_tasks[run_id] = loop.create_task(_persist_stranded_cancel(record, step_id))
