...
def build_session_status(session_name: str) -> dict:
    ...
    return {
        ...
        "epoch": (
            {"count": epoch["count"], "last_epoch_at": epoch["last_epoch_at"]} if epoch else None
        ),
        "warm_intents": [
            {
                ...
                "profile": row["worker_profile"],
                "base": row["parent_base_name"],
        ...
        "ready_bases": [
            {
                ...
                "provider": row["provider"],
                "provider_session_id": row["session_uuid"],
            }
            for row in bases
