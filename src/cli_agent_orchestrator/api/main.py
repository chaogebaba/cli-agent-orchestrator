...
class RunStepRequest(BaseModel):
    ...
    model: Optional[str] = Field(
        ...
        description=(
            ...
            "(ignored when reusing a terminal), applied ahead of the provider's "
            "existing profile/providers.toml resolution. Lets a caller pin a specific "
        ...
        if value == "mock_cli":
            ...
            if is_sandbox():
                ...
                if not getattr(MockCliProvider, "supports_reauth_rebind", False):
                    raise ValueError("provider mock_cli does not advertise supports_reauth_rebind")
                return value
...
async def lifespan(app: FastAPI):
    ...
    approval_bridge_task: Optional[asyncio.Task[None]] = None
...
async def agui_stream(
    ...
    def _sse_frames(event_id: Optional[str], frames: List[Tuple[str, Dict[str, Any]]]) -> List[str]:
...
async def agui_run(
    ...
    async def _bus_events() -> AsyncIterator[Dict[str, Any]]:
        from cli_agent_orchestrator.services.sse_bus import get_bus
...
async def create_terminal_in_session(
    ...
    ``model``: optional explicit override, applied ahead of the provider's
    existing profile/providers.toml model resolution (where the resolved provider
...
async def get_terminal_output(
    ...
    try:
        ...
        logger.exception("Output extraction failed for terminal")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except Exception as e:
...
#
# Deliberately NOT in the inbox block below.  The inbox routes are 3c's
# territory and a question notice is not an inbox concept: it becomes a delivery
# row through the composition root's notifier in slice B2, and these five routes
# are only the record's own verbs.  They are also the routes the two new MCP
# tools call, which is how standing invariant #535/F680 is satisfied by
# construction — ``ask_supervisor`` and ``cao gate ask`` reach the same handler,
# so the CLI cannot drift from the tool.
#
...
@app.get("/gate/questions/{question_id}")
...
@app.post("/gate/questions/{question_id}/answer")
...
@app.post("/gate/questions/{question_id}/escalate")
