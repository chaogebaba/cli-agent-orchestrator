...
class ClaudeCodeProvider(BaseProvider):
    ...
    def _build_claude_command(self, profile: Optional["AgentProfile"] = _UNSET) -> str:
        ...
        if profile is not None and isinstance(native, str) and native:
            ...
            if profile.mcpServers:
                ...
                if profile.inheritUserMcpServers is not True:
    ...
    def _ensure_skip_bypass_prompt_setting() -> None:
        ...
        """
        # Keep provider_home (sandbox plane) rather than Path.home(); absorb
        # upstream's lock + mode-preserving atomic write under that path.
    ...
    async def initialize(self) -> bool:
        ...
        # Tests may monkeypatch this with a sync stub; only await when async.
        _startup = self._handle_startup_prompts(outer_timeout=init_timeout)
        if inspect.isawaitable(_startup):
            await _startup
    ...
    def _after_dispatch_commit_locked(self) -> None:
        ...
        try:
            ...
            # Snapshotting is a conservative staleness guard, not a reason to reject
            # delivery when pane history is transiently unavailable.
            output = ""
