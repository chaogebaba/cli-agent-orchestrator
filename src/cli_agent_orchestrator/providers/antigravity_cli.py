    ...
    def _register_mcp_servers(self, mcp_servers: dict) -> None:
        ...
        with _MCP_CONFIG_WRITE_LOCK:
            ...
            for server_name, server_config in mcp_servers.items():
                ...
                # bind_mcp_server_identity sets CAO_TERMINAL_ID (+ sandbox
                # identity); keep that over a hand-rolled env dict.
                entry = bind_mcp_server_identity(
                    {"command": command, "args": args, "env": dict(cfg.get("env", {}))},
                    self.terminal_id,
                )
                # Per-terminal key so concurrent inits don't overwrite each
                # other's entry before agy reads the config at startup.
