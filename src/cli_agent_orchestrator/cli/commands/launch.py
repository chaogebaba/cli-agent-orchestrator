...
def _parse_env_pairs(pairs):
    ...
    try:
        result = validate_forwarded_env(parsed)
    except ForwardedEnvError as exc:
        raise click.ClickException(f"--{exc}") from exc
    # Fork-specific: CAO_ARTIFACTS_DIR must be an absolute path. Not part of the
    # shared forwarded-env contract (upstream forwarded_env.py has no such rule),
    # so it stays here on the CLI boundary after the shared rules pass.
    artifacts_dir = result.get("CAO_ARTIFACTS_DIR")
    if artifacts_dir is not None and (not artifacts_dir or not Path(artifacts_dir).is_absolute()):
        raise click.ClickException(
