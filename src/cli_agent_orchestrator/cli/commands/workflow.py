...
def runs_cmd(state, limit, as_json):
    ...
    if limit is not None:
        params["limit"] = limit
    try:
        response = cao_http.get("/workflows/runs", params=params, timeout=MCP_REQUEST_TIMEOUT)
    except requests.exceptions.RequestException as e:
        raise click.ClickException(f"could not reach cao-server: {e}")
...
def result_cmd(run_id, as_json):
    ...
    try:
        response = cao_http.get(f"/workflows/runs/{run_id}/result", timeout=MCP_REQUEST_TIMEOUT)
    except requests.exceptions.RequestException as e:
        raise click.ClickException(f"could not reach cao-server: {e}")
...
def _events_route_or_run_missing(run_id: str) -> click.ClickException:
    ...
    """
    try:
        probe = cao_http.get(f"/workflows/runs/{run_id}", timeout=MCP_REQUEST_TIMEOUT)
