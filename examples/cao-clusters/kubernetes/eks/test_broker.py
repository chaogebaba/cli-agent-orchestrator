        ...
        if STATE["create_pods"]:
            ...
            pod = k8s.V1Pod(
                metadata=k8s.V1ObjectMeta(
                    name=body.metadata.name + "-abcde", labels=dict(body.metadata.labels)
                ),
...
check(
    "job persists the authorized callback receiver",
    annotations["cao.aws/callback-terminal-id"] == "abc12345",
)
check(
    "job persists the authorized memory session",
    annotations["cao.aws/session-name"] == "cao-worker-deadbeef",
)
check(
    "job persists the authorized memory profile",
...
check(
    "failed worker's job is released",
    f"cao-worker-{w0}" in STATE["deleted_jobs"],
...
with TestClient(broker.app) as c:
    ...
    wid = lease["worker_id"]
    check(
        "job created first, then service",
    ...
    check(
        ...
        r.status_code == 200
        and any(w["worker_id"] == wid and w["state"] == "leased" for w in r.json()),
        r.text[:300],
    )
    ...
    check(
        "completing releases the job",
        f"cao-worker-{wid}" in STATE["deleted_jobs"],
        str(STATE["deleted_jobs"]),
    )
    r = c.get("/workers", headers=H)
    check(
    ...
    check(
        "reaper released the squatting job",
        f"cao-worker-{wid2}" in STATE["deleted_jobs"],
        str(STATE["deleted_jobs"]),
    )
