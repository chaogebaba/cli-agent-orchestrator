...
- `sessionBrief` (`required` or `optional`): fork-only live world-model delivery.
  Required mode fails closed when core inventory cannot be generated; use
  `--allow-incomplete-brief` only as a per-launch diagnostic escape hatch.
