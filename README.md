...
- Fork-only live context: profiles may set `sessionBrief: required` or
  `sessionBrief: optional`. Required delivery fails closed when core inventory
  cannot be generated; `--allow-incomplete-brief` is a per-launch diagnostic
  escape hatch. `CAO_SESSION_BRIEF_RELAX=1` provides the same temporary escape,
  and `CAO_SOURCE_REPO` pins the source checkout used for activation comparisons.
